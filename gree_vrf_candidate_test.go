...
func TestGreeVRFCandidateProjectsOnlyBoundedOpaqueStateCells(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		{name: "opcode 10 pair", id: 0x1ee00010, payload: []byte{0x05, 0xa1, 0xa2}, cells: []GreeVRFCandidateStateCell{{Cell: 0x0f, Value: 0xa1}, {Cell: 0x10, Value: 0xa2}}},
		{name: "opcode 10 singleton", id: 0x1ee00010, payload: []byte{0x20, 0xa3}, cells: []GreeVRFCandidateStateCell{{Cell: 0x11, Value: 0xa3}}},
		{name: "opcode 52 packed bit", id: 0x1ee00052, payload: []byte{0x59, 0x01}, cells: []GreeVRFCandidateStateCell{{Cell: 0x12, Value: 1}}},
		{name: "opcode 58 first pair", id: 0x1ee00058, payload: []byte{0x5d, 0xa4, 0xa5}, cells: []GreeVRFCandidateStateCell{{Cell: 0x13, Value: 0xa4}, {Cell: 0x14, Value: 0xa5}}},
		{name: "opcode 58 second pair", id: 0x1ee00058, payload: []byte{0x5f, 0xa6, 0xa7}, cells: []GreeVRFCandidateStateCell{{Cell: 0x15, Value: 0xa6}, {Cell: 0x16, Value: 0xa7}}},
		{name: "opcode 58 third pair", id: 0x1ee00058, payload: []byte{0x59, 0xa8, 0xa9}, cells: []GreeVRFCandidateStateCell{{Cell: 0x17, Value: 0xa8}, {Cell: 0x18, Value: 0xa9}}},
		{name: "opcode 58 fourth pair", id: 0x1ee00058, payload: []byte{0x5b, 0xaa, 0xab}, cells: []GreeVRFCandidateStateCell{{Cell: 0x19, Value: 0xaa}, {Cell: 0x1a, Value: 0xab}}},
		{name: "opcode 11 singleton", id: 0x1ee00011, payload: []byte{0x09, 0xac}, cells: []GreeVRFCandidateStateCell{{Cell: 0x1b, Value: 0xac}}},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if projection.Evidence.Frame.ID().Value() != testCase.id || projection.Class8 != 0xf7 || projection.Opaque7 != 0 || projection.Unit7 != 8 {
...
func TestGreeVRFCandidateFailsClosedForNonCandidateGeometry(t *testing.T) {
	...
	empty, err := canbus.NewFrame(mustGreeCandidateID(t, 0x1ee00010), nil)
	if err != nil {
		t.Fatal(err)
	}
	for _, testCase := range []struct {
		...
		{name: "wrong unit", evidence: greeCandidateEvidence(t, identity, 0x1ee00090, []byte{0x05, 0xa1, 0xa2})},
		{name: "wrong opcode", evidence: greeCandidateEvidence(t, identity, 0x1ee00012, []byte{0x05, 0xa1, 0xa2})},
		{name: "zero dlc", evidence: Evidence{Frame: empty, Interface: identity, Sequence: 1, Monotonic: time.Second}},
		{name: "uncovered coordinate", evidence: greeCandidateEvidence(t, identity, 0x1ee00010, []byte{0x06, 0xa1})},
	} {
		t.Run(testCase.name, func(t *testing.T) {
